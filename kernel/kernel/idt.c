...
static void setIntr(struct GateDescriptor *ptr, uint32_t selector, uint32_t offset, uint32_t dpl) {
	// TODO_ok: 初始化interrupt gate
	ptr->offset_15_0 = offset & 0xFFFF;       // 偏移量低16位
    ptr->segment = selector;                  // 段选择子
    ptr->pad0 = 0;                            // 对应声明中的 pad0 (8位)
    ptr->type = INTERRUPT_GATE_32;            // 0xE
    ptr->system = 0;                          // S位，门描述符固定为0
    ptr->privilege_level = dpl;               // DPL (0 或 3)
    ptr->present = 1;                         // P位，设为1
...
static void setTrap(struct GateDescriptor *ptr, uint32_t selector, uint32_t offset, uint32_t dpl) {
	// TODO_ok: 初始化trap gate
ptr->offset_15_0 = offset & 0xFFFF;
    ptr->segment = selector;
    ptr->pad0 = 0;
    ptr->type = TRAP_GATE_32;                 // 0xF
    ptr->system = 0;
